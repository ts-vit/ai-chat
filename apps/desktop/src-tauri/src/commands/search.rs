...
pub async fn search_messages(
    ...
    let provider = get_embedding_provider(&app).await;
    if let (Some(store), Some(p)) = (store.as_ref().as_ref(), provider.as_ref()) {
        hybrid_search::hybrid_search(pool.inner(), Some(store), query, 50, p.as_ref()).await
    } else {
        // FTS-only fallback
...
pub async fn delete_search_index(
    ...
    if let Some(s) = store.as_ref().as_ref() {
        s.delete_by_chat_id(&chat_id).await.map_err(|e| e.to_string())?;
    }
    sqlx::query("UPDATE messages SET fts_indexed = 0 WHERE chat_id = ?")
        .bind(&chat_id)
        .execute(pool.inner())
        .await
        .map_err(|e| e.to_string())?;
    let _ = sqlx::query("DELETE FROM messages_fts WHERE message_id IN (SELECT id FROM messages WHERE chat_id = ?)")
        .bind(&chat_id)
        .execute(pool.inner())
        .await;
    Ok(())
...
pub async fn delete_message_index(
    ...
    sqlx::query("UPDATE messages SET fts_indexed = 0 WHERE id = ?")
        .bind(&message_id)
        .execute(pool.inner())
        .await
        .map_err(|e| e.to_string())?;
