...
use uni_llm::{AttachmentInput, ChatRequest, ContentBlock, Message, SseEvent, SseParser};
    ...
    let rag_sources_json: Option<String> = if !chat_id.is_empty() && rag_mode_str != "model_only" {
        ...
        if let (Some(ref kb_id_val), Some(store)) = (&kb_id, kb_vector_store.as_ref().as_ref()) {
            ...
            let kb_row = sqlx::query(
                "SELECT name, embedding_model, embedding_dimensions, retrieval_top_k, retrieval_min_score, \
            ...
            if let Some(kb_row) = kb_row {
                ...
                let embedding_model: String = kb_row.try_get("embedding_model").unwrap_or_else(|_| "openai".to_string());
                let embedding_dimensions: i64 = kb_row.try_get("embedding_dimensions").unwrap_or(1536);
                ...
                if !user_query.is_empty() {
                    ...
                    let provider = crate::commands::knowledge_base::build_provider_for_embedding_model(
                        &app,
                        &embedding_model,
                    )
                    .await?;
                    ...
                    match crate::services::kb_search_orchestrator::search_kb_orchestrated(
                        ...
                        Some(embedding_dimensions as i32),
                    )
                    .await
                    {
    ...
    let stream_result = tokio::select! {
        ...
        r = async move {
            ...
            'tool_loop: for _iteration in 0..MAX_TOOL_ITERATIONS {
                ...
                let app_cb = app_stream.clone();
                let sse_result = match SseParser::parse_stream(
                    ...
                    |event| {
                        match event {
                            ...
                            SseEvent::ImageDelta { url } => {
                                pending_images.push(url.clone());
                            }
                            _ => {}
                        }
                    },
                ...
                full_content.lock().await.push_str(&sse_result.content);

                if let Some(ref msg_id) = assistant_msg_id {
                    for data_url in &pending_images {
                        ...
                        if let Some(base64_str) = data_url.splitn(2, ',').nth(1) {
                            match base64::engine::general_purpose::STANDARD.decode(base64_str.trim())
                            {
                                Ok(decoded) => {
                                    ...
                                    if let Ok(rel_path) =
                                        save_attachment_file(&app_stream, msg_id, &file_name, &decoded)
                                    {
                                        let _ = app_stream.emit(
                                            "chat-stream-image",
                                            StreamImagePayload {
                ...
                // Tool call handling
                if (sse_result.got_tool_calls_finish || !sse_result.tool_calls.is_empty())
                    && !token_check.is_cancelled()
                {
                    log::debug!(
                        "send_message: {} tool_call(s) in iteration {}",
                        sse_result.tool_calls.len(),
                        _iteration
                    );

                    let content_value = if sse_result.content.is_empty() {
                        serde_json::Value::Null
                    } else {
                        serde_json::Value::String(sse_result.content.clone())
                    };

                    let tc_array: Vec<serde_json::Value> = sse_result
                        .tool_calls
                        .iter()
                        .map(|buf| {
                            serde_json::json!({
                    ...
                    for buf in &sse_result.tool_calls {
