...
mod tests {
    ...
    async fn test_project_summary_counts() {
        ...
        // Add 1 memory
        insert_memory(&pool, &uuid::Uuid::new_v4().to_string(), "remember this", "fact", Some(&project_id)).await;
    ...
    async fn test_delete_project_removes_memory_and_plans() {
        ...
        insert_memory(&pool, &uuid::Uuid::new_v4().to_string(), "fact 1", "fact", Some(&project_id)).await;

        let now = chrono::Utc::now().timestamp();
        sqlx::query(
            "INSERT INTO agent_plans (id, chat_id, goal, status, execution_mode, created_at, updated_at, project_id) VALUES (?, ?, 'goal', 'draft', 'manual', ?, ?, ?)",
        )
        .bind(uuid::Uuid::new_v4().to_string()).bind(&chat_id).bind(now).bind(now).bind(&project_id)
        .execute(&pool).await.unwrap();
