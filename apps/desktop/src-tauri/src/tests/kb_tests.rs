...
mod tests {
    ...
    async fn test_list_knowledge_bases() {
        ...
        for (i, name) in ["KB A", "KB B", "KB C"].iter().enumerate() {
            ...
            sqlx::query(
                "INSERT INTO knowledge_bases (id, name, description, embedding_model, created_at, updated_at) VALUES (?, ?, '', 'openai', ?, ?)",
            )
    ...
    async fn test_delete_document_cascades_chunks() {
        ...
        let c1 = uni_common::generate_id();
        let c2 = uni_common::generate_id();
