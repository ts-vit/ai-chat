...
fn map_kb_row(row: sqlx::sqlite::SqliteRow) -> crate::models::knowledge_base::KnowledgeBase {
    crate::models::knowledge_base::KnowledgeBase {
        ...
        embedding_dimensions: row.try_get("embedding_dimensions").unwrap_or(1536),
...
async fn spawn_indexing(
    ...
    let provider =
        crate::commands::knowledge_base::build_provider_for_embedding_model(
            app_handle,
            &kb.embedding_model,
        )
        .await?;
...
pub async fn create_notebook(
    ...
    let notebook_id = uni_common::generate_id();
    let kb_id = uni_common::generate_id();
    ...
    sqlx::query(
        "INSERT INTO knowledge_bases (id, name, description, embedding_model, embedding_dimensions, \
         ...
         VALUES (?, ?, '', 'openai', 1536, 'tokens', 512, 50, 50, 5, 0.7, \
