...
fn chunks_to_batch(chunks: &[ChunkRecord], schema: &Schema, embedding_dim: i32) -> Result<RecordBatch> {
    let n = chunks.len();
    ...
    let mut all_embeddings: Vec<f32> = Vec::with_capacity(n * dim);
    for c in chunks {
...
impl VectorStore {
    ...
    /// Drop Lance table if existing vector dimension does not match configured dimension.
    ...
    pub async fn save_chunks(&self, chunks: Vec<ChunkRecord>) -> Result<()> {
        ...
        let batch = chunks_to_batch(&chunks, &schema, dim)?;
        let names = conn.table_names().limit(1000).execute().await?;
