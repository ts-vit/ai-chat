...
pub async fn generate_plan(
    ...
    let resp_json = uni_llm::complete(&client, base, &api_key, &body)
        .await
        .map_err(|e| format!("Plan generation request failed: {}", e))?;

    let content = uni_llm::extract_content(&resp_json)
    ...
    let task_ids: Vec<String> = raw_tasks.iter().map(|_| uni_common::generate_id()).collect();
...
async fn replan(
    ...
    let content = uni_llm::extract_content(&resp_json)
        .ok_or_else(|| "No content in replan response".to_string())?;
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if ready_tasks.len() == 1 {
                ...
                let next_assistant_msg_id = uni_common::generate_id();
                crate::commands::agent::agent_loop(
                    app.clone(),
                    pool.clone(),
                    mcp_manager.clone(),
