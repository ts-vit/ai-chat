...
pub async fn create_memory_impl(
    ...
    let embedding = if let Some(engine) = EmbeddingEngine::get() {
...
pub async fn search_memories_impl(
    ...
    crate::services::memory_search::search_memories(pool, memory_store, query, limit, project_id).await
}
...
pub async fn update_agent_memory(
    ...
    if let Some(ref c) = content {
        ...
        if let Some(engine) = EmbeddingEngine::get() {
            let prefixed = format!("passage: {}", c);
            if let Ok(embeddings) = engine.embed(&[prefixed]) {
...
pub async fn search_agent_memories(
    ...
    search_memories_impl(pool.inner(), memory_store.as_ref().as_ref(), &query, limit, None).await
}
