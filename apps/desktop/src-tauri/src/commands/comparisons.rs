...
use crate::commands::chat::{
    is_image_generation_model, mcp_tools_to_openai, parse_tool_call_name, StreamState,
    MAX_TOOL_ITERATIONS,
};
use uni_llm::{AttachmentInput, ContentBlock, SseEvent, SseParser};
...
async fn stream_one_side(
    ...
    'tool_loop: for _iteration in 0..MAX_TOOL_ITERATIONS {
            ...
            |event| {
                match event {
                    SseEvent::TextDelta(text) => {
                        let _ = app_cb.emit("comparison-stream", ComparisonStreamPayload {
                            side: side_cb.clone(),
                            content: text.clone(),
                        });
                    }
                    SseEvent::Usage(u) => {
                        let _ = app_cb.emit("comparison-stream-usage", ComparisonStreamUsagePayload {
                            side: side_cb.clone(),
        ...
        full_content.push_str(&sse_result.content);
...
pub async fn stream_comparison_responses(
    ...
    let left_assistant_id = uni_common::generate_id();
    sqlx::query(
    ...
    let right_assistant_id = uni_common::generate_id();
    sqlx::query(
