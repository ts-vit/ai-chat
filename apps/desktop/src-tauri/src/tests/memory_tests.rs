...
mod tests {
    ...
    async fn test_create_memory_with_project() {
        ...
        let mem_id = uni_common::generate_id();
    ...
    async fn test_list_memories_by_project() {
        ...
        insert_memory(&pool, &uni_common::generate_id(), "global", "fact", None).await;
        insert_memory(&pool, &uni_common::generate_id(), "project-specific", "fact", Some(&project_id)).await;
