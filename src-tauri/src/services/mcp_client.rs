// MCP client: one server process, JSON-RPC 2.0 over stdin/stdout
...
fn windows_command(command: String) -> String {
    if cfg!(windows) {
...
async fn run_reader(
    ...
    loop {
        ...
        let json_str: String = if trimmed.starts_with("Content-Length:") || trimmed.starts_with("content-length:") {
            ...
            if let Err(e) = reader.read_exact(&mut body).await {
                log::debug!("MCP reader: read_exact error ({} bytes): {}", content_len, e);
                break;
            }
            ...
            log::debug!("MCP reader: skip non-JSON line: {:?}", &line_buf[..n.min(200)]);
            continue;
        };
...
impl McpClient {
    ...
    async fn request(&self, method: &str, params: Option<serde_json::Value>) -> Result<JsonRpcResponse, String> {
        ...
        let response = tokio::time::timeout(Duration::from_secs(RPC_TIMEOUT_SECS), rx)
            .await
            .map_err(|_| {
            ...
            .map_err(|e| e)?;
        log::debug!("MCP response: id={} has_result={} has_error={}", id, response.result.is_some(), response.error.is_some());
    ...
    pub async fn call_tool(
        ...
        let result = response.result.ok_or("missing result")?;
        let out: McpToolResult = serde_json::from_value(result).map_err(|e| e.to_string())?;
        Ok(out)
    }
