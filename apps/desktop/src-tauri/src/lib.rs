...
pub(crate) const ALTER_QUERIES: &[&str] = &[
    ...
    "ALTER TABLE knowledge_bases ADD COLUMN embedding_dimensions INTEGER NOT NULL DEFAULT 1536",
    ...
    // API embeddings (replacing local e5): migrate metadata; LanceDB tables reset on dim mismatch
    "UPDATE knowledge_bases SET embedding_model = 'openai', embedding_dimensions = 1536 WHERE embedding_model = 'e5-small'",
];
        ...
        .setup(|app| {
            ...
            let store: Option<VectorStore> = match app.path().app_data_dir() {
                Ok(data_dir) => {
                    ...
                    match tauri::async_runtime::block_on(VectorStore::new(
            ...
            let memory_store: Option<MemoryVectorStore> = match app.path().app_data_dir() {
                Ok(data_dir) => {
                    let db_path = data_dir.join("lancedb");
                    match tauri::async_runtime::block_on(MemoryVectorStore::new(
                        db_path.to_string_lossy().as_ref(),
                        embedding_dim,
                    )) {
                        Ok(s) => Some(s),
            ...
            let _ = tauri::async_runtime::block_on(
                sqlx::query("ALTER TABLE knowledge_bases ADD COLUMN embedding_dimensions INTEGER NOT NULL DEFAULT 1536").execute(&pool)
            );
            let _ = tauri::async_runtime::block_on(
